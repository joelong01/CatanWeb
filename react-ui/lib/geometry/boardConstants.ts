...
function calculateEdgeMidpointOffsets(): Record<HexSide, { dx: number; dy: number }> {
  ...
  const result: Record<HexSide, { dx: number; dy: number }> = {} as Record<HexSide, { dx: number; dy: number }>;
...
export function getPatternId(resourceType: ResourceType): string {
  switch (resourceType) {
    ...
    case 'Wheat': return 'tile-wheat';
    case 'Wood': return 'tile-wood';
    case 'Ore': return 'tile-ore';
    case 'Sheep': return 'tile-sheep';
    case 'Desert': return 'tile-desert';
    case 'GoldMine': return 'tile-goldmine';
    case 'Sea': return 'tile-sea';
...
export function getHarborPatternId(harborType: HarborType): string {
  switch (harborType) {
    case 'Brick': return 'harbor-brick';
    case 'Ore': return 'harbor-ore';
    case 'Sheep': return 'harbor-sheep';
    case 'Wheat': return 'harbor-wheat';
    case 'Wood': return 'harbor-wood';
    case 'ThreeForOne': return 'harbor-3for1';
    default: return 'harbor-3for1';
  }
}
