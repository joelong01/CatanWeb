...
- **Removed**: `GameStateMachineWrapper.cs` and `GameServiceFactoryAdapter.cs` - unnecessary wrapper layers
- **Pattern Change**: Old lambda approach `ExecuteAction(gameId, gsm => gsm.DoSomething())` → Direct method calls
- **New Structure**: 
...
- **Centralized game creation logic** - GameFactory handles GameId, GameName, CreatedTime
...
- **GameApiController.cs:182** - Still using old lambda pattern for LoadExistingGameAsync
- **Missing method** - Need LoadFromCompressedLogAsync or similar for SerializableLog path
- **Pattern confusion** - LoadGameMessage (CompressedLog) vs LoadGameModelMessage (GameModelJson)
...
### Core Game Loading
- `Catan3.GameService/Controllers/GameApiController.cs` - REST endpoints for game lifecycle
